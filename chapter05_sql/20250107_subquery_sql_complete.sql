...
-- 2번 쿼리
(SELECT * FROM orders
	WHERE order_date >= "2025-10-01" AND order_date < "2015-11-01")
UNION ALL 
(SELECT * FROM orders
	WHERE order_date >= "2025-12-01" AND order_date < "2016-01-01")
	ORDER BY order_date DESC;
...
-- 내 풀이
SELECT *
	FROM orders o LEFT JOIN staff s ON o.user_id = s.user_id
...
-- 이상의 코드는 staff 테이블에서 id값이 3, 5를 도출해냈습니다.
...
SELECT *
	FROM ( SELECT *
			FROM orders
			WHERE order_date >= "2015-07-01"
				AND order_date < "2015=08=01") o-- 1)의 결과가 테이블이었기 때문에 별칭 o를 붙임
				INNER JOIN
				(SELECT *
					FROM orderdetails
...
-- 서브 쿼리 정리하기
-- 쿼리 결과값을 메인 쿼리에서 값이나 조건으로 사용하고 싶을 떄 사용
...
-- 특징 :
-- FROM에서 서브 쿼리를 적절히 활용하면 적은 연산으로 같은 결과를 도출 가능. 단, RDBS기분
...
-- 	WHERE에 작성하는 필터링 대상 컬럼 개수가 일치 해야 함. -> 이 떄 필터링 대상 컬럼이 2 개 이상이면
-- ()로 묶어서 작성해야 합니다.
...
-- 추출하라.
SELECT country, city, SUM(is_auth)		-- is_auth가 1이면 본인인증한거니까 is_auth들의 합이 10이렴
