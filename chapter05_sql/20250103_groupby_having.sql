...
-- 표시 컬럼 uniqueUserCnt 두 컬럼으로 구성돼있습니다.
SELECT country, count(DISTINCT id) AS uniqueUserCnt
	FROM users
	WHERE country = "Korea" AND is_marketing_agree=1;

-- users에서 country 별로 마케팅 수신 동의한 회원 수와 동의하지 않은 회원 수를 구해 출력
...
-- 따라서 '중요한 기분을 앞에 배치' 해서 시각화와 그룹화에 대한 우선순위를 결정할 필요 있음.

-- GROUP BY 정리
-- 1) 주어진 컬럼을 기준으로 데이터를 그룹화하여 '집계힘수'와 함께 사용
-- 2) GROUP BY 뒤에는 그룹화할 컬럼명을 입력. 그룹화한 컬럼에 집계 함수 적용하여
		-- 그룹별 계산을 수행.
-- 3) 형식 : GROUP BY 컬럼명 1, 컬럼명 2 ...
...
-- 2. orders에서 staff_id별, user_id 별로 주문 건수(COUNT(*))를 출력할 것
...
-- 1. where에서 필터링을 시도할 떄 오류가 발생하는 이유:
