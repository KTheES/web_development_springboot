...
-- user와 order를 하나로 결합하여 출력합니다(단 주문 정보가 있는 회원의 정보만 출력) 
SELECT *
	FROM users u 
...
-- 개별 행을 구분해야 하므로 null값을 가질 수 없습니다
...
-- 세 번 주문했다면 회원 정보(users)와 주문 정보(orders)를 결합한 결과에는 u.id =7 인
-- 행에 3 개 있을 것이다. 이 경우 ' 한 번 이라고 주문한 회원 수'를 중복 없이 구하려면 회원
...
-- FK는 다른 테이블의 고유한 키 값인 PK를 찹조한다.(orders에서 o.id가 FK에 해당,
-- users의 u.id가 PK에 해당해서 FK가 PK를 참조해서 조건을 합치시켜 JOIN함.) 

-- 예를 들어서 PK값이 A, B, C만 있다면 PK값도 이 값만 가질 수 있고, 또한 중복되지 않는다는
-- 특징을 지니고 있음.
...
-- 이를 JOIN 시켰을 때 u.id로 ORDER BY 하더라도 합친 테이블 상에서 제 1컬럼(즉 PK컬럼)에
...
-- 첫 번째 JOIN의 ON절 뒤에, 두번쨰 JOIN 절을 작성하면 됩니다.
...
-- 결합으로 봤을 떄 orders는 users 테이블에 종속돼있으므로 null 값이 출력될 수 없습니다.
...
-- 성, 이름을 한 화면에 출력하세요
SELECT u.id, u.username, u.country, u.city, u.country, s.last_name, s.first_name
...
SELECT s.user_id, s.last_name, s.first_name, o.id ,o.order_date
	FROM staff s LEFT JOIN orders o ON s.user_id =o.user_id
...
SELECT u.country,COUNT(DISTINCT o.id)
	...
	ORDER BY COUNT(DISTINCT o.id) ASC
	;
...
SELECT o.user_id
	...
	ORDER BY sum(p.price * quantity) ASC
	;
...
-- 일단 JOIN 이 수행된 뒤에 다른 명령어들이 수행됩니다. JOIN사용 시, 결합할 두 테이블 사이에
-- 원하는 JOIN 명령어를 작성하고, 테이블 별칭을 설정(알리아스가 아닙니다.).
-- 또한 두 테이블 사이에 공통된 컬럼 값인 키 값이 존재해야만 JOIN으로 결합할 수 있습ㄴ미다.
...
-- LEFT JOIN : 왼쪽 테이블의 모든 데이터 값을 결과에 포함(보통 많이 쓰임)
...
-- CROSS JOIN은 두 테이블을 결합했을 떄 각 테이블의 행으로 만들 수 있는 모든 조합을 결과값으로
