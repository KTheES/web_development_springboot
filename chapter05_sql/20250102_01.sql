...
-- 뿐만 아니라 특정 컬럼의 값이 A인 데이터만 거져오는 등 복잡한 쿼리를
-- 작성할 일이 있습니다.

-- 회원 정보 테이블 uesrs에서 거주 국가(country)가 한국(Korea)인
-- 회원만 추출하는 쿼리문
...
-- 응용 : users에서 거주국가가 KOREA, USA ,UK인 회원 정보를 추출하세요.
...
SELECT created_at, phone, city, country
	...
	-- 2. products에서 id가 20 이하이고 price는 30 이상인 제품 정보 출력
		-- 단 기존 컬럼 전부 출력하고, 정상 가격에서 얼마나 할인 되었는질,
...
SELECT id, name, price
	...
	WHERE name LIKE "N%"
	;
	-- 5. orders에서 order_date가 2015-07-01부터 2015-10-31 까지가
		-- 아닌 정보만 출력할 것
SELECT *
	FROM orders
...
-- 정렬할 떄 사용 : 숫자의 경우는 쉽게 알 수 있지만, 문자의 경우는
...
SELECT * 
	FROM 음식점정보
	WHERE 지역 = "마포구"
		AND 1인분가능여부 =1;		-- 1을 쓸 경우에는 True 의미 /0은 F 의미
