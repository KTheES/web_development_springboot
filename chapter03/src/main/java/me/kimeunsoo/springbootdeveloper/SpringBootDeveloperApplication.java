...
/*
        ...
        SpringBoot의 등장
         ...
         차이점 :
            1) 구성의 차이 -> 스프링은 애플리케이션 개발에 필요한 환경을 수동으로 구성하고 정의
                하지만 스프링부트는 스프링 코어와 스프링 MVC 의 모든 기능을 자동으로 로드
    ...
    2. 스프링 컨셉
        1) 제어의 역전 (IoC)
            Inversion of Control :다른 객체를 직접 생성하거나 제어하는 것이 아니라
                ...
                ex ) 클래스 A에서 클래스 B의 객체를 생성한다고 가정했을 떄의 자바 코드
                public class A{
                    B b = new B();
                }
    ...
    5) 이식 가능한 서비스 추상화 (PSA)
        Portable Service Abstraction : 스프링에서 제공하는 다양한 기술들을 '추상화' 헤
            개발자가 쉽게 사용하는 '인터페이스'
            ...
            WAS 역시 PSA 의 예시 중 하나로, 어떤 서버를 사용하더라도(톰캣, 언더토우, 네티 등)
            코드를 동일하게 작성 가능
...
3. 스프링 부트 3 둘러보기
    첫 번째 스프링 부트 3 예제 만들기
     ...
     스프링 부트 스타터 살펴보기
        ...
        spring-boot-starter-validation : 유효성 검사를 위해 필요한 의존성 모등
        spring-boot-starter-actuator : 모니터링을 위해 애플리케이션에서 제공하는 다양한 정보를
            제공하기 쉽게 하는 의존성 모음
        spring-boot-starter-jpa : ORM(추후 수업 예정)을 사용하기 위한 인터페이스이 모음인
