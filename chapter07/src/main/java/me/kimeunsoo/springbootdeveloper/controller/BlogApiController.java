...
public class BlogApiController {
    ...
    // HTTP 메서드가 POST일 때 전달 받은 URL과 동일하면 지금 정의하는 메서드와 매핑
    ...
    /*
        ...
        2. @PostMapping() : HTTP 메서드가 POST일 떄 요청 받은 URL과 동일한 메서드와 매핑.
        ...
        API가 잘 동작하는지 하나 테스트를 해볼 예정

            H2 콘솔 활성화

            application.yml으로 이동
        ...
        SQL statement 입력창에(SQL 편입기모양인데에)
        select * from article
        안된 분 혹시 ARTICLE
        그리고 Run 눌러서 쿼리 실행
        h2 데이터베이스에 저장된 데아터를 확인할 수 있습니다,
        ...
        내일은 반복작업을 줄여줄 테스트 코드들을 작성하겠습니다.
            ...
            클래스명 클릭 + alt + enter -> create test 가 있었습니다.
    */
    ...
    /*
        /api/articles GET 요청이 들어오면 글 전체를 조회하는 finaAll() 메서드를 호출
        -> 다음 응답용 객체인 ArticleResponse로 파싱해서 body에 담아
        클라이언트에게 전송합니다 -> 해당 코드에서는 stream을 적용했습니다 -> 추후 설명

        * stream : 여러 데이터가 모여 있는 컬렉샨을 간편하게 처리하기 위해서 사용하는 기능
            자바 8에 추가
     */
    ...
    /*
        ...
        제목 1부터 내용 3까지의 JSON 데이터가 있는지 확인하고,
        ...
        다 확인 끝났으면 BlogApiControllerTest.java 가서 테스트형태의 메서드로 삭제 확인
     */
    ...
    /*
        /api/articles/{id} PUT 요청이 들어오면 Request Body 정보가 request로 넘어옵니다.
