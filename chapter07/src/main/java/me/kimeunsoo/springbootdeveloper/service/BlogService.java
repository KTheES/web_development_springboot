...
public class BlogService {
    ...
    /*
        ...
        -> 컨트롤러 메서드 코드 작성하기
            ...
            이름에서 유추할 수 있는 각 애너테이션들은 HTTP 메서드와 대응합니다.
    ...
    /*
        트랜젝션 - 데이터베이스에서 데이터를 바꾸기 위한 작업 단위를 의미함.

            예를 들어 계좌 이체를 할 때 이런 과정을 거친다고 가정합니다.
                1) A계좌에서 출금
                2) B계좌에 입금
            그런데 1)은 성공했는데, 도중에 2)가 실패하면 어떻게 되나요? 고객 입장에서는 출금음
            됐는데, 입금이 안된 상황이 발생합니다.
            이런 상황이 발생하지 않으려면 출급과 입금을 하나의 작업 단위로 묶어서,
