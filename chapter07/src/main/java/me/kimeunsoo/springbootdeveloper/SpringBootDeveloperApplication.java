...
/*
    1. 사전 지식 : 타임리프(Thymeleaf)
        ...
        * 템플릿 엔지 : 스프링 서버에서 데이터를 받아 우리가 보는 웹 페이지, 즉 HTML 상에서
            그 데이터를 넣어 보여주는 도구.
        ...
        템플릿 엔진 개념 잡기
            ...
            *{...} - 선택한 변수의 표현식. th:object에서 선택한 객체에 접근
            ...
            타임리프 문법
            th:text - 텍스트를 표현할 떄 사용 - th:text=${person.name}
            th:each - 컬렉션을 반복할 때 사용 - th:each="person:${persons}"
            th:if - 조건이 true인 떄만 표시 - th:if"${person.age}>=20"
            th:unless - 조건이 false인 떄만 표시 - th:unless"${person.age}>=20"
