...
5장. '데이터베이스' 조작이 편해지는 'ORM'
        ...
        DBMS(Database Management System) : 기본적으로 데이터베이스는 많은 사람이 공유할 수 있어야 하고,
            ...
            H2, MySQL
                ...
                H2 - 자바로 작성되어 있는 RDBMS  : 스프링 부트가 지원하는 인메모리 관계형 데이터베이스
                    ...
                    그래서 애플리케이션 재실행시 데이터는 초기화된(서버 재실행을 기준으로).
                    간편하게 사용하기 좋아서 테스트용도로 자주 사용됨.
                    실제 서비스에서는 사용하지 않습니다.
                MySQL - 실제 서비스로 올릴 떄 사용할 RDBMS -> 추후 수업 예정
            ...
            필수 용어
                ...
                3) 열(column) : 테이블의 구성 요소 중 하나이며, 행에 저장되는 유형의 데이터.
                    ...
                    무결정을 보장함. 이상의 표를 기분으로 할 때는, 이메일은 문자열일 것, 나이는 숫자 유형을 가짐
                    이메일 열에 숫자가 들어가거나, 나이 열에 문자열이 들어갈 수 없기 떄문에 데이터에 대한 무결성을 보장.
                4) 기본키(Primary key) : 행을 구분할 수 있는 식별자. 이 값은 테이블에서 유일해야 하며 '중복값을 가질 수 없음'
                    보통 데이터를 수정하거나 삭제하고, 조회할 때 사용되며 다른 테이블과 관계를 맺어 데이터를 가져올 수도 있음(JOIN 개념)
    ...
    2. ORM이란?
        Object-Relational Mapping이라고 하며 자바의 객체와 데이터베이스를 연결하는 프로그래밍 기법
            ...
            즉, SQL에 어려움을 격는다고 하더라도 자바 언어로만 데이터베이스에 접근해서 원하는 데이터를 받아올 수 있는 방식.
        ...
        장점 :
            ...
            4) 매핑하는 정보가 명확하기 떄문에(DB값과 인스턴스 변수의 값), ERD에 대한 의존도를 낮출 수 있고 유지보수시 유리
