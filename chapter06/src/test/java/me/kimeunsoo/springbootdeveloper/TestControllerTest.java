...
class TestControllerTest {
    ...
    public void getAllMembers() throws Exception {       // throws Exception : 예외로 처리함
        ...
        // maybeags/web_development_java 에 c15_casting에 CentralControll.java/ Main.java 확인
...
/*
    ...
    (3) : andExpect() 메서드는 응답을 검증. TestController에서 만든 API는 응답으로
        OK(200)을 반환하므로 이에 해당하는 메서드의 isOk()를 사용해 응답 코드가 200(OK)인지 확인

    (4) : jsonPath("$[0].{필드명})은 JSON응답값의 값(을 가져오는 역할을 하는 메서드.
