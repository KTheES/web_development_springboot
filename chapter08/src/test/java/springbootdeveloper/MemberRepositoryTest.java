...
import me.kimeunsoo.springbootdeveloper.Member;
import me.kimeunsoo.springbootdeveloper.MemberRepository;
...
/*
    ...
    test -> resources 우클릭, insert-members.sql 파일 생성
    작성 후(혹은 확인 후에) 코드 의미 :
...
class MemberRepositoryTest {
    ...
    /*
        ...
        member라는 테아블의 모든 컬럼을 조회하는데, id가 2인 객체의 값들을 보여달라.
    ...
    /*
        ...
        MemberRepostiory에 findByName 정의 후에 이하에 작성
     */
    @Sql("/insert-members.sql")
    @Test
    void gerMemberByName9(){    // MemberRepository.java에는 findByName()이었다는 점에 주족
        //when
    ...
    /*
        ...
        INSERT INTO member (id, nem) VALUES (1,'A'); 라는 쿼리가 있다고 가정했을 때
        JPA에서는 save()라는 메서드를 사용
     */

    @Test       //@Sql 애너테이션을 달지 않습니다 이 메서드에서 1, 'A'에 해당하는 객체를 저장할거라서
    void saveMember(){
        //given
    ...
    /*
        ...
        DELETE FRO+M member WHERE id = 2;
    ...
    /*
        ...
        JPA에서 데이터를 수정하는 방식은 추후 SQL에서 배우게 될 트랜잭션
        내에서 데이터를 수행해야 합니다.
    ...
    @Test
    void update(){
        //given -> id 2인 객체을 가지고 와서 member 라는 객체명에 저장 / 현재 name = "B"
